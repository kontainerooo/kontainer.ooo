...
func (s *service) StartWebsocketTransport(errc chan error, logger log.Logger, wsAddr string) {
	...
	wss := ws.NewServer(s.ProtocolMap, logger, s.WebsocketUpgrader, s.TokenAuth, s.SSLConfig, s.ErrorHandler, ws.Before(s.BartBus.GetOff), ws.Before(s.BartBus.LostAndFound), ws.After(s.BartBus.GetOn))
